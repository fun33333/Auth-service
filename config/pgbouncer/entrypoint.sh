...
FINAL_DB_HOST=${RESOLVED_IP:-$DB_HOST}

# Generate pgbouncer.ini
cat <<EOF > /tmp/pgbouncer.ini
[databases]
auth_db = host=${FINAL_DB_HOST} port=${DB_PORT} dbname=auth_db
hdms_db = host=${FINAL_DB_HOST} port=${DB_PORT} dbname=hdms_db
sms_iak_db = host=${FINAL_DB_HOST} port=${DB_PORT} dbname=sms_iak_db
postgres = host=${FINAL_DB_HOST} port=${DB_PORT} dbname=postgres
