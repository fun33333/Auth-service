...
function BranchModal({ open, onClose, onSave, initial }: {
  ...
  const empty = { branch_name: "", branch_code: "", institution_code: "", city: "", address: "", email: "", contact_number: "", status: "active", branch_head_name: "" };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {([
            { label: "Branch Name", key: "branch_name", placeholder: "e.g. Head Office Lahore" },
            { label: "Branch Code", key: "branch_code", placeholder: "e.g. BR-007" },
            { label: "Institution Code", key: "institution_code", placeholder: "e.g. AIT" },
            ...
            { label: "Phone", key: "contact_number", placeholder: "+92-XX-XXXXXXX" },
            { label: "Branch Head", key: "branch_head_name", placeholder: "e.g. Dr. Ali" },
          ...
          <div>
            ...
            <select value={form.status === "active" ? "Active" : "Inactive"}
              onChange={e => setForm(p => ({ ...p, status: e.target.value === "Active" ? "active" : "inactive" }))}
...
export default function BranchesPage() {
  ...
  const stats = [
    ...
    { label: "Active", value: branches.filter(b => b.status === "active").length, icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Inactive", value: branches.filter(b => b.status !== "active").length, icon: XCircle, color: "text-rose-600", bg: "bg-rose-50" },
    { label: "Total Branches", value: branches.length, icon: Users, color: "text-violet-600", bg: "bg-violet-50" },
  ];

  async function handleSave(data: Partial<Branch>) {
    // For now do local update; actual create/update API can be added later
    if (editTarget) {
      ...
      <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map(s => {
            const Icon = s.icon;
            return (
              <div key={s.label + s.color} className="relative group overflow-hidden bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.08)] hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-white">
                <tr>
                  {["Branch", "Institution", "Location", "Contact", "Status", ""].map(col => (
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {loading ? [...Array(4)].map((_, i) => (
                  ...
                  <tr key={b.branch_id} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="whitespace-nowrap py-4 pl-6 pr-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 shrink-0 rounded-xl bg-violet-100 text-violet-700 flex items-center justify-center font-bold text-sm">{b.branch_name?.charAt(0)}</div>
                        <div>
                          <div className="font-semibold text-slate-900 text-sm">{b.branch_name}</div>
                          <div className="text-xs font-mono text-slate-400">{b.branch_code}</div>
                        </div>
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4">
                      <div className="flex items-center gap-1.5 text-sm text-slate-700"><Building2 className="h-3.5 w-3.5 text-slate-400" />{b.institution_code}</div>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4">
                    ...
                    <td className="whitespace-nowrap px-4 py-4">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-slate-400 mt-0.5"><Phone className="h-3 w-3" />{b.contact_number || '—'}</div>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${b.status === 'active' ? "bg-green-100 text-green-800" : "bg-red-100 text-red-700"}`}>
                        <span className={`h-1.5 w-1.5 rounded-full ${b.status === 'active' ? "bg-green-500" : "bg-red-500"}`} />
                    ...
                    <td className="whitespace-nowrap py-4 pl-4 pr-6">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => setDeleteId(b.branch_id)} className="p-2 text-slate-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
